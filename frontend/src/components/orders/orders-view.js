...
export class OrdersView {
    ...
    showOrder(order) {
        ...
        document.getElementById('completed').innerText = order.completeDate ? (new Date(order.completeDate)).toLocaleDateString('ru-RU') : '(Заказ не выполнен)';
        ...
        if (order.freelancer.avatar) {
            document.getElementById('freelancer-avatar').setAttribute('src', config.host + order.freelancer.avatar);
        }
